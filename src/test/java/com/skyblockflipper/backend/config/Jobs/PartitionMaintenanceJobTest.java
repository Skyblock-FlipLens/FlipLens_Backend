...
class PartitionMaintenanceJobTest {
    ...
    void compactAggregatePartitionsMaterializesBucketsBeforeRetention() {
        ...
        InOrder callOrder = inOrder(materializationService, lifecycleService);
        callOrder.verify(materializationService).materializeDueBuckets(any(Instant.class));
        callOrder.verify(lifecycleService).executeAggregateRetention(any(Instant.class));
    }
    ...
    void compactAggregatePartitionsSkipsMaterializationWhenUnavailableOrDisabled() {
        ...
        verify(lifecycleService, times(2)).executeAggregateRetention(any(Instant.class));
    }
